...
def filter_text_streams(queue_text1, queue_text2, queue_cancel, config):
    ...
    while True:
        ...
        while not queue_text1.empty():
            buffer.append(queue_text1.get())

        # Figure out times
        now = datetime.utcnow()
        sendwindow_start = now - datetime.timedelta(seconds=config_filter["send_delay_max"])
        sendwindow_stop = now - datetime.timedelta(seconds=config_filter["send_delay_min"])
...
def get_words_in_window(buffer, sendwindow_start, sendwindow_stop):
    ...
    while index >= 0:
        if buffer[index][1] < sendwindow_start:
            ...
            # Too new, change index to look at a different item
            index -= 1
